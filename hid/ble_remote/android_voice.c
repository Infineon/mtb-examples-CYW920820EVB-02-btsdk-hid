...
extern void bleremoteapp_handleVoiceTxCommand(uint8_t command);
/*
...
//        WICED_BT_TRACE("\nATVV_%s_CLIENT_CFG_DESC: len=%d:", p_read_data->handle == HANDLE_ATV_VOICE_RX_CLIENT_CONFIGURATION_DESCRIPTOR ? "RX" : "CTL");
        ...
        TRACE_ARRAY(p_read_data->p_val + p_read_data->offset, (uint32_t) *p_read_data->p_val_len);
        return WICED_BT_GATT_SUCCESS;
...
wiced_bt_gatt_status_t android_voice_write_handler(uint16_t conn_id, wiced_bt_gatt_write_t *p_write_data)
{
    ...
    case HANDLE_ATV_VOICE_TX_CHARACTERISTIC_VALUE:
    {
        ...
        command = *p++;
        len--;
        WICED_BT_TRACE("\nATVV_CHAR_TX cmd: %d ", command);

        if (command == ATV_VOICE_SERVICE_GET_CAPS_REQUEST)
        {
            WICED_BT_TRACE("CAPS_REQUEST");
            if (len < 4)
            {
