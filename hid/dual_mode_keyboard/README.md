...
- ble\_keyboard:
    - Turnkey BLE Keyboard solution using on-chip keyscan HW component, based on HID over GATT Profile (HOGP).
- ble_mouse:
    - BLE Mouse application is a single chip SoC solution based on HID over GATT Profile (HOGP).
- ble_remote:
    - BLE Remote Control application is a single chip SoC solution based on HID over GATT Profile (HOGP).
