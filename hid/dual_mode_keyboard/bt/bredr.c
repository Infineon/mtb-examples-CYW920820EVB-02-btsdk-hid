...
static const UINT8 wiced_bt_sdp_db[] =
{
    ...
    SDP_ATTR_SEQUENCE_2(262 + SPD_RPT_DESCRIPTOR_SIZE +                 //HID Device
                        162),                                           //Device ID

    // First SDP record HIDD: Keyboard
    SDP_ATTR_SEQUENCE_2(259 + SPD_RPT_DESCRIPTOR_SIZE),                 // 3 bytes for length of the record + length of the record
    ...
    SDP_ATTR_ID(ATTR_ID_HID_DESCRIPTOR_LIST), SDP_ATTR_SEQUENCE_2(SPD_RPT_DESCRIPTOR_SIZE+8),
        SDP_ATTR_SEQUENCE_2(SPD_RPT_DESCRIPTOR_SIZE+5),                 // 3 bytes ==>
        SDP_ATTR_VALUE_UINT1(0x22),                                     // 2 bytes ==> 0x08, 0x22(Report)
        SDP_ATTR_VALUE_TEXT_2(SPD_RPT_DESCRIPTOR_SIZE),                 // 3 bytes ==> 0x25, SPD_RPT_DESCRIPTOR_SIZE,
        USB_RPT_DESCRIPTOR,
...
#if !is_208xxFamily     // power management patch is not in 208xx
/*****************************************************************************
...
/********************************************************************************
 * Function Name: void BREDR_write_eir
 ********************************************************************************
 * Summary: Initialization for EIR (Extended Inquiry Response) data
 *
 * Parameters:
 *  dev_name  - The device name to be put in EIR
 *
 * Return:
 *  none
 *
...
#if 0
    // print EIR data
    STRACE_ARRAY("\nEIR: ", (uint8_t*) pBuf, p - (uint8_t*) pBuf);
    WICED_BT_TRACE(" (\"%s\")", dev_name);
#endif
    wiced_bt_dev_write_eir(pBuf, (uint16_t) (p - pBuf));
}

/********************************************************************************
 * Function Name: void BREDR_stateChangeNotification()
 ********************************************************************************
 * Summary: BR/EDR link state change callback function
 *
 * Parameters:
 ...
 *
 *******************************************************************************/
STATIC void BREDR_transportStateChangeNotification(uint32_t newState)
{
    app_transportStateChangeNotification(BT_TRANSPORT_BR_EDR, (uint8_t) newState);
}
...
void bredr_init()
{
    ...
    WICED_BT_TRACE("\nsdp_db %02x %d",wiced_bt_sdp_db[0], sizeof(wiced_bt_sdp_db));
...
// #if !is_208xxFamily     // power management patch is not in 208xx
 #if 0 // need to work on 20735 wiced_bt_hidd_configure_power_management_params() access
    ...
    /* initialize eir */
    BREDR_write_eir(BT_LOCAL_NAME);

    hidd_btlink_add_state_observer(BREDR_transportStateChangeNotification);
